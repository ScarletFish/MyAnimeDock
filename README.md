...
- **mpv 播放与进度追踪**：通过 IPC 管道实时追踪进度，每 10 秒落盘、关闭时最终保存；播放结束弹出完工确认（可设自动/关闭），支持断点续播。
- **完整状态管理**：覆盖导入到完结的全周期，支持 5 种状态、评分、进度、日期与笔记。
- **统计看板**：完善的数据统计页面，展示标签词云、观看活跃度、评分分布与季度分布等。
- **剧集缩略图**：后台 ffmpeg 队列生成剧集缩略图，启动时自愈补全缺失项，播放时自动暂停避免抢占。
- **数据库管理**：设置页内置备份 / 全量备份 / 恢复 / 缓存清理 / VACUUM 优化，重置前自动备份。
- **主题系统**：6 种色彩主题 + 独立深色/浅色模式，底部 Dock 一键切换。
- ~~**Bangumi 双向同步**：授权后 Pull → Merge → Push，每次操作自动同步状态。~~
  因 Bangumi 原站网络访问受限，该功能已**雪藏**，暂无入口。
...
- **前端** — Svelte 5 + Vite + Tailwind CSS v4（SPA）
- **桌面壳** — Tauri v2（Rust）
