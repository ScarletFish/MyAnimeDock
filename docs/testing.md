...
```bash
cd server && npm test            # 全量测试（当前 368 tests: 35 DB + 64 scanner + 65 scrapers + 44 playback 等）
...
```
server/__tests__/
├── db.test.js            # 数据库集成测试（35 tests）
├── scanner.test.js       # Scanner 纯函数 + 文件系统集成（64 tests）
├── scrapers.test.js      # 元数据匹配逻辑（65 tests）— 改匹配时只跑这个
└── playback-encoding.test.js  # 播放路径编码验证（44 tests）
```
