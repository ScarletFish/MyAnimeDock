...
**网格卡片尺寸基准**: `GRID_CARD_BASE = 240`（`library.js`），实际渲染尺寸 = `240 × --scale`（= 详情封面宽度）。`repeat(auto-fill, minmax(size, 1fr))` 填满行宽，卡片自身 `max-width` 限制拉伸，`justify-items: center` 居中。仪表盘各模块以此为锚点对齐。
