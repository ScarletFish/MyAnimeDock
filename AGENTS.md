...
**网格卡片尺寸基准**: `GRID_CARD_BASE = 220`（`library.js`），实际渲染尺寸 = `220 × --scale`（= 详情封面 92%）。`repeat(auto-fill, size)` 固定列宽 + `justify-content: center` 居中。gap = `calc(1.875rem × --scale)`。仪表盘各模块以此为锚点对齐。
