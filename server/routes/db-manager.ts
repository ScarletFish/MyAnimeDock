// server/routes/db-manager.ts — 数据库管理：信息、备份、恢复、清除、重置、优化
import * as path from 'path';
import * as fs from 'fs';
import { PROJECT_ROOT } from '../lib/paths';
import { jsonResp, readBody } from '../lib/utils';
import { DATA_DIR, CONFIG_PATH, SCANNED_TREE_PATH } from '../lib/config';
...
function dirSize(dirPath: string): { size: number; files: number } {
  try {
...
async function handleDbInfo(req: any, res: any, state: State): Promise<void> {
  const { data } = state;
  const info = {
    dbPath: DB_FILE,
    ...
    counts: {
      ...
      episodes: (data?.library || []).reduce((sum: number, a: any) => sum + (a.episodes?.length || 0), 0),
...
function handleDbBackup(req: any, res: any): void {
...
async function handleDbBackupAll(req: any, res: any): Promise<void> {
  // 收集所有需要打包的文件
  ...
  try {
    ...
    jsonResp(res, 500, { error: '打包失败: ' + (e as Error).message });
  }
}

// POST /api/db/restore — 恢复数据库（上传 .db 文件，base64 编码）
async function handleDbRestore(req: any, res: any, state: State): Promise<void> {
  try {
    ...
    jsonResp(res, 500, { error: '恢复失败: ' + (e as Error).message });
  }
}

// POST /api/db/clear-sessions — 清除播放记录
async function handleDbClearSessions(req: any, res: any, state: State): Promise<void> {
  try {
    const body = await readBody(req);
    ...
    jsonResp(res, 500, { error: '清除失败: ' + (e as Error).message });
  }
}

// POST /api/db/vacuum — SQLite VACUUM 优化
async function handleDbVacuum(req: any, res: any, state: State): Promise<void> {
  try {
    ...
    jsonResp(res, 500, { error: 'VACUUM 失败: ' + (e as Error).message });
  }
}

// POST /api/db/clear-cache — 清除缓存文件（封面/缩略图/横幅）
async function handleDbClearCache(req: any, res: any, state: State): Promise<void> {
  try {
    const body = await readBody(req);
    ...
    jsonResp(res, 500, { error: '清除缓存失败: ' + (e as Error).message });
  }
}

// POST /api/db/reset — 重置数据库（清空所有数据）
async function handleDbReset(req: any, res: any, state: State): Promise<void> {
  try {
    ...
    jsonResp(res, 500, { error: '重置失败: ' + (e as Error).message });
  }
}
